...
def Sell(address,slippage):
    global active, selling, Sold, approvedToken, startedSelling
    selling = True
    bsc = "https://speedy-nodes-nyc.moralis.io/855ba305c4dc5aec35810d1e/bsc/mainnet"
    web3 = Web3(Web3.HTTPProvider(bsc,websocket_timeout=3))
    ...
    amount = str(balance)
    amount = amount[:decimal] + "." + amount[decimal:]
    symbol = sellTokenContract.functions.symbol().call()
    tokenValue = web3.toWei(float(amount), 'ether')
    ...
    approvedToken = str(("Approved: " + web3.toHex(tx_token)))
    time.sleep(3)
    startedSelling = str((f"Swapping {tokenValue2} {symbol} for BNB"))
    ...
    tx_token = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
    Sold = str((f"Sold {symbol}: " + web3.toHex(tx_token)))
    del variables[thread_numbers[address]]
...
def trackPrice(address):
    while True:
        try:
            global thread_number, variables
            thread_number = thread_number + 1
            thread_numbers[address] = {"thread":thread_number}
            trailing = False
            data = requests.get("https://deep-index.moralis.io/api/v2/erc20/"+str(address)+"/price?chain=bsc&exchange=PancakeSwapV2",headers={"X-API-Key": "HZjuPqq6ecLUZyBWElxdk0L06zbNhyCeWpvSrKlMeyXyUR5fTFAvnkh0We0ruDuR"})
            info = requests.get("https://api.pancakeswap.info/api/v2/tokens/"+str(address))
            print(data.json())
            print(info.json())
            highest_string = "{:.25f}".format(float(data.json()["usdPrice"]))
            highest = float(highest_string)
            name = info.json()["data"]["name"]
            symbol = info.json()["data"]["symbol"]
            stop = float(highest) * 0.8
            target = float(highest) * 1
            variables[thread_numbers[address]["thread"]] = {"Name": str(name),
                                        "Symbol": str(symbol),
                                        "Stop": str("{:.25f}".format(float(stop))),
                                        "Target": str("{:.25f}".format(float(target))),
                                        "Price": highest_string,
                                        "Contract": address}
            while True:
                try:
                    data = requests.get("https://deep-index.moralis.io/api/v2/erc20/"+str(address)+"/price?chain=bsc",headers={"X-API-Key": "HZjuPqq6ecLUZyBWElxdk0L06zbNhyCeWpvSrKlMeyXyUR5fTFAvnkh0We0ruDuR"})
                    price_string = "{:.25f}".format(float(data.json()["usdPrice"]))
...
web3 = Web3(Web3.WebsocketProvider("wss://speedy-nodes-nyc.moralis.io/855ba305c4dc5aec35810d1e/bsc/mainnet/ws",websocket_timeout=3))
web3.middleware_onion.inject(geth_poa_middleware, layer=0)

while True:
    try:
        while web3.isConnected() == True:
            transactions = web3.eth.get_block("latest",full_transactions=True)
            block_number = transactions["transactions"]
            block_number = block_number[0]["blockNumber"]
            stdscr.erase()
            stdscr.addstr(banner)
            stdscr.addstr("Checking Block: "+str(block_number)+"\n")
            for i in active:
                number = thread_numbers[i]["thread"]
                name = variables[number]["Name"]
                symbol = variables[number]["Symbol"]
                stop = variables[number]["Stop"]
                target = variables[number]["Target"]
                price = variables[number]["Price"]
                stdscr.addstr("==================================================\n")
                stdscr.addstr("Now Tracking "+str(name)+" / "+str(symbol)+"\n")
                stdscr.addstr("Stop Loss: "+str(stop)+"\n")
                stdscr.addstr("Take Profit: "+str(target)+"\n")
                stdscr.addstr("Current Price: "+str(price)+"\n")
                stdscr.addstr("==================================================\n")
                if selling == True:
                    stdscr.addstr(approvedToken)
                    stdscr.addstr(startedSelling)
                    stdscr.addstr(Sold)
            stdscr.refresh()
            for i in transactions["transactions"]:
                ...
                if sent_from == wallet:
                    for i in range(1,(thread_number+1)):
                        if variables[i]["Contract"] == sent_to:
                            duplicate = True
                    if duplicate == False:
                        x = threading.Thread(target=trackPrice, args=(sent_to,), daemon=True).start()
                        active.append(sent_to)
    except:
        pass
